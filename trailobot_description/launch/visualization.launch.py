import os

from ament_index_python.packages import get_package_share_directory
from launch import LaunchDescription
from launch.actions import DeclareLaunchArgument, OpaqueFunction
from launch.substitutions import LaunchConfiguration
from launch_ros.actions import Node

import xacro

package_description = "trailobot_description"

def process_xacro(context):
    robot_type_value = LaunchConfiguration('robot_type').perform(context)
    pkg_path = get_package_share_directory(package_description)
    xacro_file = os.path.join(pkg_path, 'urdf', 'robot.xacro')
    
    # Process the xacro file with the robot_type mapping
    doc = xacro.process_file(xacro_file, mappings={'robot_type': robot_type_value})
    return doc.toxml()


def launch_setup(context, *args, **kwargs):
    robot_description = process_xacro(context)
    
    return [
        Node(
            package='robot_state_publisher',
            executable='robot_state_publisher',
            name='robot_state_publisher',
            output='screen',
            parameters=[{
                'robot_description': robot_description,
                'use_sim_time': True,
                'publish_frequency': 50.0,
                'use_tf_static': True,
            }],
        ),
        Node(
            package='joint_state_publisher_gui',
            executable='joint_state_publisher_gui',
            name='joint_state_publisher',
            output='screen',
        )
    ]


def generate_launch_description():
    # Launch argument
    robot_type_arg = DeclareLaunchArgument(
        'robot_type',
        default_value='trailobot',
        description='Type of the robot'
    )

    rviz_config_file = os.path.join(
        get_package_share_directory(package_description),
        'rviz',
        'config.rviz'
    )

    return LaunchDescription([
        robot_type_arg,
        OpaqueFunction(function=launch_setup),
        Node(
            package='rviz2',
            executable='rviz2',
            name='rviz2',
            output='screen',
            arguments=['-d', rviz_config_file],
        )
    ])

...
def generate_launch_description():
    pkg_trailobot = get_package_share_directory('trailobot_launch')
    
    # Launch configurations
    use_sim_time = LaunchConfiguration('use_sim_time', default='true')
    autostart = LaunchConfiguration('autostart', default='true')
    map_yaml_file = LaunchConfiguration('map', default=os.path.join(pkg_trailobot, 'map', 'current_map.yaml'))
    params_file = LaunchConfiguration('params_file', default=os.path.join(pkg_trailobot, 'config', 'nav2_param_usage.yaml'))
    ...
    declare_map_yaml = DeclareLaunchArgument(
        'map',
        default_value=os.path.join(pkg_trailobot, 'map', 'current_map.yaml'),
        description='Full path to map yaml file to load'
    )
    
    declare_params_file = DeclareLaunchArgument(
        'params_file',
        default_value=os.path.join(pkg_trailobot, 'config', 'nav2_param_usage.yaml'),
